...
export default function MangaReader() {
  ...
  const { mangaProvider } = useApiConfig();
  const [mode, setMode] = useState<'scroll' | 'page'>('scroll');
  const [currentPage, setCurrentPage] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const resolvedMangaId = decodeParam(mangaId);
  const resolvedChapterId = decodeParam(chapterId);

  const { data: pages, isLoading } = useQuery({
    queryKey: ['manga-read', resolvedChapterId, mangaProvider],
    queryFn: () => mangaApi.read(resolvedChapterId, mangaProvider),
    enabled: !!resolvedChapterId,
  });

  const { data: mangaInfo } = useQuery({
    queryKey: ['manga-info-reader', resolvedMangaId, mangaProvider],
    queryFn: () => mangaApi.info(resolvedMangaId, mangaProvider),
    enabled: !!resolvedMangaId,
  });

  const chapters = mangaInfo?.chapters || [];
  const currentChIdx = chapters.findIndex(c => c.id === resolvedChapterId);
  ...
  useEffect(() => { setCurrentPage(0); }, [resolvedChapterId, mangaProvider]);
          ...
          <div className="flex items-center gap-2">
            {prevCh && (
              <Button variant="outline" size="sm" asChild>
                <Link to={`/read/${encodeURIComponent(resolvedMangaId)}/${encodeURIComponent(prevCh.id)}`}>
            ...
            {nextCh && (
              <Button variant="outline" size="sm" asChild>
                <Link to={`/read/${encodeURIComponent(resolvedMangaId)}/${encodeURIComponent(nextCh.id)}`}>
      ...
      <div className="mx-auto max-w-[900px] p-4">
        {!pages || pages.length === 0 ? (
          ...
          <div className="space-y-1">
            {pages.map((p, i) => (
              <img key={i} src={p.img} alt={`Page ${p.page}`} className="w-full" loading="lazy" referrerPolicy="no-referrer" />
            ))}
          </div>
        ) : (
